...
router.get("/login", (req, res) => {
  res.oidc.login({
    returnTo: "/authenticate",
  });
});

router.get("/authenticate", requiresAuth(), async (req, res) => {
  let alUser;
  try {
    alUser = await User.findOne({ email: req.oidc.user.email });
  } catch (err) {
...
router.get("/create-account", requiresAuth(), async (req, res) => {
  let alUser;
  try {
    alUser = await User.findOne({ email: req.oidc.user.email });
  } catch (err) {
...
router.post("/create-account", requiresAuth(), async (req, res) => {
  ...
  try {
    ...
    console.error("Error creating user: " + err);
    res.redirect("/create-account?err=Error creating account, please try again");
    return;
  }

  const newUserSettings = new UserSettings({ userEmail: req.oidc.user.email });

  try {
    await newUserSettings.save();
  } catch (err) {
    console.error("Error creating user settings: " + err);
    User.findOneAndRemove({ email: req.oidc.user.email })
      .catch((err) => {
        console.error("Error removing newly created user: " + err);
        res.redirect("/create-account?err=Concurrent Exception while repairing error, please report this as a bug");
      });
    res.redirect("/create-account?err=Error creating account, please try again");
    return;
  }
