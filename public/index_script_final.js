...
function turnRegistered(eventsToDisable) {
  for (var i = 0; i < eventsToDisable.length; i++) {
    ...
    document.getElementById(eventsToDisable[i].id).innerHTML =
        ...
        for (var i = 0; i < eventsW2.length; i++) {
          if (eventsW2[i].id == datarray[datarray.length - 1]) {
            ...
            document.getElementById("registerTitle").innerText =
              "Choose a carpool · " + day + " at " + eventTime;
          }
        }
          ...
          if (obj["nameOfEvent"] == datarray[datarray.length - 1]) {
            ...
            document.getElementById("registerJS").innerHTML +=
              `<div class="notification"  style="padding: 10px;   margin-bottom: 10px;">
                                                  <div class="button  selectCarpool" style=" margin-left: 10px; margin-top: 4px; background-color: transparent; float: right" onclick=" selectCarpool(this)" id="` +
              obj["id"] +
              `">Select</div>
                                                          <div style="float: right; margin-top: 3px;"  >` +
              obj["carpoolers"].length +
              `/` +
              obj["seats"] +
              ` signed up</div>
                                                                                    Organizer: <a style="color: #3273dc" href="mailto:` +
              obj["email"] +
              `">` +
              obj["email"] +
              `</a>
...
document.body.onmousedown = function () {
  if (document.getElementById("address-input") != null) {
    ...
    document
      .getElementById("autocomplete-container2")
      .classList.remove("is-loading");
  }
};
var arr = [];
...
addressAutocomplete(
  document.getElementById("autocomplete-container2"),
  "address-input2",
  (data) => {
    console.log("Selected option: ");
    console.log(data);
